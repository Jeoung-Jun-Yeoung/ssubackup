...
쓰레드 함수로 백업파일리스트 인자로 넘겨주는것 성공.

탐색 쓰레드는 계속 백업파일리스트를 보면서 리스트에있는 파일들을 백업시켜주는 쓰레드를 만들어줌.

로그파일 작성하는 코드를 구현하고 있는데,  쓰레드로 빼야 하나 고민중. 

백업파일하는 쓰레드가 문제가 생겨서 디버깅중.

remove 쓰레드도 디버깅완료.


### 20210303 study
백업쓰레드함수 구현완료해서 디버깅하는중에, 백업파일이 백업폴더에 생기는게 아니라 original file이 있는곳에 생기고 있었음.

경로설정에서 문제가 있어서 이부분 표시해서 다시 구현중. 

log파일 작성하는 함수는 쓰레드로 안짰는데,  쓰레드로 만들어야 할것 같아서 다시 만드는중.
...
경로설정 문제 해결.
로그파일 구현완료.
remove에서 코어덤프오류가 발생함. 해결해야 할듯.
그리고 compare함수구현하면 될것 같다.
